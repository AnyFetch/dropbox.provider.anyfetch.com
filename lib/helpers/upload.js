...
var uploadFile = function(identifier, metadata, dropboxTokens, anyfetchClient, cache, cb) {
  ...
  retrieve.getFile(dropboxTokens, metadata.path, cache, function(status, reply) {
    if(status !== 200 || !reply) {
      return cb(new Error("Failure to retrieve data or empty file: " + [identifier, status, dropboxTokens].toString()));
    }
 ...
 */
module.exports = function(task, anyfetchClient, dropboxTokens, cache, cb) {
  ...
  return uploadFile(identifier, task.metadata, dropboxTokens, anyfetchClient, cache, loggerCb);
};
