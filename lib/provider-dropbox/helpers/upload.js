    ...
    var stack = tokens.map(function(token) {
      return function(cb) {
        retrieve.delta(token.dropboxTokens, token.cursor, function(err, files) {
          ...
          var fileQueue = files.files.map(function(path) {
            return {
              path: path,
          ...
          createQueue.push(fileQueue);

          var deleteFileQueue = files.deletedFiles.map(function(path) {
            return {
              path: path,
              dropboxTokens: token.dropboxTokens,
              cluestrClient: cluestr
          ...
          deleteQueue.push(deleteFileQueue);
