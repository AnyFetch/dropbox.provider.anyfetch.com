...
exports.callback = function (req, res, next) {
  async.waterfall([
    ...
    function(tempToken, cb) {
      dbApp.accesstoken(tempToken.dropboxTokens, function(status, accessToken) {
        if(status !== 200) {
          return cb(new Error("Bad Dropbox status: " + status));
        }

        cb(null, tempToken, accessToken);
      });
    },
    function(tempToken, accessToken, cb) {
      // Save for future access in MongoDB
      var token = new Token({
        cluestrToken: tempToken.cluestrCode,
