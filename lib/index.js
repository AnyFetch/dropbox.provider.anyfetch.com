...
var updateAccount = function(serviceData, cursor, queues, cb) {
  ...
  retrieveFiles(serviceData, cursor, function(err, entries, newCursor) {
    ...
    entries.forEach(function(entry) {
      if(!entry[1]) {
        queues.deletion.push({title: "Add " + path.basename(entry[0]), path: entry[0], metadata: entry[1]});
      }
      else {
        queues.addition.push({title: "Del " + path.basename(entry[0]), path: entry[0], metadata: entry[1]});
      }
    });
...
var additionQueueWorker = function(job, cb) {
  uploadFile(job.task, job.anyfetchClient, job.serviceData, job.cache, cb);
};
